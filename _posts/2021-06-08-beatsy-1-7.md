...
titleVideo:
    file: 'title.mp4'
links:
  - title: "Get Beatsy on the App Store"
...
However the update also includes a few new features for small screen users. For starters, the UI now respects light and dark appearance instead of hardcoding a dark UI. I also split up the capture button, so now there's one button for stills and one for video. This should improve discoverability of video recording. 

And while on the subject, I've also completely rewritten how video recording is implemented. Video recording should now perform much better, even on older devices like iPhone 6s. You can also now easily trim captured videos before saving or sharing them.
